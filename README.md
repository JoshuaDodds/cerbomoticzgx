...
- **AI Optimization Configuration**:
  ...
  - `ESS_ADAPTIVE_POLICY_ENABLED=False`: Summer-only attended-validation gate. When enabled, Trading, PV-first self-sufficiency and protected hybrid are compared through the same continuous-horizon production optimizer. Trading must beat the best conservative candidate by `ESS_ADAPTIVE_TRADE_MIN_BENEFIT_EUR` plus the configured fraction of learned forecast risk; the raw error is bounded by `ESS_ADAPTIVE_FORECAST_RISK_MAX_EUR`, and common forecast error is not charged a second time after lifecycle/arbitrage risk. The selected policy replans every optimizer cycle, while all three are re-scored every `ESS_ADAPTIVE_FULL_EVALUATION_INTERVAL_MIN` (default 60), when Tibber extends/corrects the price horizon, or after a material load/PV, SoC or EV-block change. This bounds compute without keeping a stale strategy or using a coarser, energy-inventing SoC lattice. Strategy dwell/switch-margin settings prevent flapping. `ESS_ADAPTIVE_UNKNOWN_HORIZON_HOURS` caps both protected continuation and terminal energy credit, using the trailing time-of-day load rate instead of diluting overnight need with daytime PV. `ESS_ADAPTIVE_POLICY_STATE_PATH` persists selection/evaluation state across restarts. This does not affect the separate Winter engine.
  ...
  - `AC_DC_CHARGE_EFFICIENCY` / `AC_DC_DISCHARGE_EFFICIENCY`: Configured one-way conversion estimates (both default 0.96, matching all code paths). Their combined 92.16% round-trip estimate is not a measured site calibration; validate it from clean metered cycles before changing production values.
  - `MIN_SOC_RESERVE_WINTER` / `MIN_SOC_RESERVE_SUMMER`: Optimizer planning reserve (%) selected explicitly by `WINTER_MODE` (defaults 40 / 5). On this 42 kWh bank, 40% reserves 16.8 kWh for an unexpected winter outage and leaves 25.2 kWh tradeable, versus 33.6 kWh at 20% (25% less tradeable capacity). This deliberate safety buffer is released only after an explicit Victron grid-offline signal, which suppresses optimizer control and permits emergency discharge toward zero. These logical floors do not write Victron's hard minimum.
  ...
  - `ESS_EXPORT_PRICE_FACTOR` / `ESS_EXPORT_FEE`: Export price model — `sell = buy * factor - fee`. This installation defaults to `1.0 / 0.0248` for Tibber NL during 2026 while annual imported energy still covers exports under saldering. The optimizer does not yet track the contract-year allowance, and this model must not be assumed valid for net overproduction or after saldering ends on 2027-01-01; settled Tibber reward remains authoritative.
  - `ESS_TERMINAL_VALUE_FACTOR`: Scales a bounded continuation value on every truncated price horizon, including same-day-only data. Only trailing time-of-day household need for `ESS_ADAPTIVE_UNKNOWN_HORIZON_HOURS` is credited, at the visible upper-quartile avoided-import price (or the explicit expected peak if higher); excess stored energy receives no invented terminal windfall. `0` disables the entire continuation credit.
  - `ESS_EXPECTED_PEAK_PRICE`: Optional explicit continuation/avoided-import value floor (currency/kWh); 0 disables this floor and leaves the bounded upper-quartile model authoritative.
  ...
  - `ESS_COST_BASIS_PATH`: Path for persisted stored-energy cost basis. Grid charging raises the basis and PV charging dilutes it. Because historical acquisition cost is sunk, the dynamic floor waits for the best remaining visible sale but cannot strand energy when recovery is impossible; `ESS_MIN_SELL_PRICE` remains the absolute operator floor.
  - **Executable schedule invariant:** the five Victron charge-window limit is part of the DP state, not a post-processing truncation. Every planned grid BUY is therefore covered by a published window. Excess forecast PV beyond battery/export capacity is represented as `pv_curtailed_kwh`; it never creates fictitious export revenue or makes an otherwise physical plan infeasible.
...
- **ESS strategy counterfactual**: `python scripts/evaluate_ess_strategies.py --plan /dev/shm/cerbo_ai_plan.json --json` compares a frozen plan against simplified market-arbitrage, PV-first self-sufficiency, and protected-hybrid candidates. Its human report uses dashboard terminology: **Grid result** is export reward minus import cost; **After battery wear** additionally subtracts estimated battery cycle cost. It reads only the supplied plan/configuration snapshot and never imports the live broker, writes MQTT/Victron, or chooses a dispatch policy. After upgrading, run one optimizer replan first so the exported plan includes its explicit assumptions; an older plan can instead be examined only with the deliberately labelled `--use-research-defaults` option. It is research evidence, not a second controller.
  ...
  - **PV surplus is stored, not sold, while the battery has room.** The optimizer never imposes an export setpoint on PV surplus (see `_post_process` in `lib/ai_powered_ess.py`): the setpoint stays neutral and the Victron stores surplus until the battery cannot accept more. The live Summer DP and the evaluator therefore reject a non-discharging export while another complete SoC step can fit without importing; any remaining sub-step surplus is conservatively absorbed up to headroom/rate rather than credited as meter export. `summarize_steps` applies the same storage-first settlement rule. A commanded battery discharge is untouched. `stored_surplus_kwh` reports what a row stored instead of selling. `PV_SURPLUS_FULL_SOC` lives in `lib/forecast_projection.py` so the live plan, dashboard and evaluator cannot drift apart.
  - **`plan_baseline` is the comparison row.** The active plan's own per-slot flows are re-totalled by the same function as the candidates, so the only difference left between rows is the policy. It is withheld (`available: false`) rather than estimated when the published plan lacks `grid_energy`/`soc_start`/`soc_end`; whole-day totals are likewise withheld when the plan carries no `today_actuals`. Do **not** score candidates against the dashboard `day_summary` tile: that tile fraction-weights the active slot and combines settled actuals with only the remaining day; mixing that basis with a full-horizon candidate previously overstated alternatives by more than €10 by counting tomorrow's revenue as today's.
  ...
  - The optimizer runs every 15 minutes and again at 13:05 (after next-day Tibber prices publish) over one continuous known horizon. The removed calendar-midnight split could double-count first-day sacrifice, compare raw cash instead of the DP objective and allocate Victron windows independently. `planning_policy.reason_code=UNIFIED_HORIZON_OBJECTIVE` identifies the repaired path. Each run classifies the current slot into **IDLE**, **RETAIN**, **BUY**, or **SELL** with a plain-English reason and machine-readable `reason_code`. Inspect it without applying anything via `python scripts/ai_ess_dryrun.py`.
