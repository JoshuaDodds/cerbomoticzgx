...
- **Web dashboard** (`frontend/`): a self-contained, read-only visibility dashboard (Flask) showing the current decision, an expandable hour→15-min→reasoning schedule tree, the day cost summary (actuals + forecast), and current configuration. Runs as its own process/sidecar (`python -m frontend`) or an optional in-process thread. See `frontend/README.md`.
- **AI Advisor** (dashboard "Advisor" tab): a manually-triggered, **read-only** Claude review of recent optimizer behaviour. Run the default daily review or ask an open question ("Why did we sell at 15:00 yesterday?"). It analyses recent history + the allow-listed tunables (never secrets) and recommends improvements, classifying each as an existing-tunable change, a new tunable, or a code change. It cannot change anything. Auth uses your Claude **Pro/Max subscription** via Claude Code (`claude setup-token` → `CLAUDE_CODE_OAUTH_TOKEN` in `.secrets`, no API key needed) or an `ANTHROPIC_API_KEY` fallback. (Phase 1 of a planned advise→approve→apply workflow.)
