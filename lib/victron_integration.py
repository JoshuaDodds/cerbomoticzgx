...
from lib.constants import logging, Topics, TopicsWritable, dotenv_config
...
def ac_power_setpoint(watts=None, override_ess_net_mettering=True):
    ...
    if watts:
        ...
        publish_message(TopicsWritable['system0']['ac_power_setpoint'], message=_msg, retain=False)

def minimum_ess_soc(percent: int = 10):
    if percent:
        _msg = f"{{\"value\": {percent}}}"
        logging.info(f"Setting battery sustain percent to: {percent}%")
        publish_message(TopicsWritable['system0']['minimum_ess_soc'], message=_msg, retain=False)
...
def regulate_battery_max_voltage(ess_soc):
    ...
    try:
        if int(ess_soc) == float(dotenv_config('MINIMUM_ESS_SOC')) and current_max_charge_voltage != float_voltage:
            publish_message(TopicsWritable["system0"]["max_charge_voltage"], payload=f"{{\"value\": {float_voltage}}}", retain=False)
            ...
            publish_message(TopicsWritable["system0"]["max_charge_voltage"], payload=f"{{\"value\": {max_voltage}}}", retain=False)
            ...
            publish_message("Tesla/vehicle0/solar/ess_max_charge_voltage", payload=f"{{\"value\": \"{max_voltage}\"}}", retain=True)
            ...
            publish_message(TopicsWritable["system0"]["max_charge_voltage"], payload=f"{{\"value\": \"{battery_full_voltage}\"}}", retain=False)
            ...
            publish_message("Tesla/vehicle0/solar/ess_max_charge_voltage", payload=f"{{\"value\": \"{battery_full_voltage}\"}}", retain=True)
            ...
            publish_message("Tesla/vehicle0/solar/ess_max_charge_voltage", payload=f"{{\"value\": \"{current_max_charge_voltage}\"}}", retain=True)
